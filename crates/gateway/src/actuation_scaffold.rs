...
//! knowledge) → plant receives command locally and writes only **ACK/NACK** CAN frames (or no
//! response); gateway read loop decodes those outcomes into [`PhysicalCarVocabulary`] like telemetry.
//!
...
use crate::devices::corner_lights::can::{actuation_command_wire_meta, encode_ack_frame, encode_nack_frame};
...
/// Simulated body ECU: consumes commands from the channel, logs command receipt, then writes
/// response CAN frames (`ACK` or `NACK`) onto `can_interface` (same bus as telemetry).
///
    ...
    tokio::spawn(async move {
        ...
        while let Some(cmd) = cmd_rx.recv().await {
            let command_direction = match &cmd {
                ActuationCommand::SwitchCornerLightsOn { .. } => "ON",
                ActuationCommand::SwitchCornerLightsOff { .. } => "OFF",
            };
            let (session, seq) = actuation_command_wire_meta(&cmd);
            eprintln!(
                "[corner-light-actuator]: received {command_direction} command (wire session={session} seq={seq})"
            );
            ...
            eprintln!(
                "[corner-light-actuator]: responding with {} for {command_direction} command (wire session={session} seq={seq})",
